  ...
  methods: {
    async fetchAreasFuncionales() {
      ...
      try {
        ...
        // Accede a la propiedad "data" en la respuesta
        this.areasFuncionales = response.data.data || [] // Asigna los datos del array
        this.pagination.rowsNumber = response.data.total || 0 // Total de filas para la paginación
      } catch (error) {
    ...
    async saveItem() {
      try {
        if (this.currentItem.id) {
          await this.$axios.put(`http://104.131.72.170:3333/api/v1/areas/${this.currentItem.id}`, this.currentItem)
        } else {
          await this.$axios.post('http://104.131.72.170:3333/api/v1/areas', this.currentItem)
        }
    ...
    async deleteItem() {
      try {
        await this.$axios.delete(`http://104.131.72.170:3333/api/v1/areas/${this.itemToDelete}`)
